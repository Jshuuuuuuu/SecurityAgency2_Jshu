    ...
    pool.getConnection()
        .then(conn => {
            conn.query('SELECT * FROM users WHERE email = ?', [email])
                ...
                .catch(err => {
                    res.status(500).json({ success: false, message: 'Error checking login credentials' });
...
// Route to handle registration (email + password)
            ...
            conn.query('SELECT * FROM users WHERE email = ?', [email])
                .then(rows => {
                    if (rows.length > 0) {
                        ...
                        conn.query('INSERT INTO users (email, password_hash) VALUES (?, ?)', [email, hash])
                            .then(result => {
